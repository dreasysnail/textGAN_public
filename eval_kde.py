...
def log_mean_exp(A,b,sigma):
    a=-0.5*((A-b)**2).sum(1)/(sigma**2)
    max_=a.max()
    return max_+np.log(np.exp(a-max_).mean())

"""calculate negative KDE"""
def cal_nkde(X,mu,sigma):
    #print 'in KDE calculation'
    E=mean([log_mean_exp(mu,x,sigma) for x in X])
    #E=0.0
    #for x in X:
    #    E=E+log_mean_exp(mu,x,sigma)
    Z=mu.shape[1]*np.log(sigma*np.sqrt(np.pi*2))
    return Z-E # Not sure about the denominator
...
if __name__ == '__main__':
    ...
    print 'Calculate median distance'
    sigma = np.sqrt(np.median([((x-y)**2).sum() for x in emb_test for y in emb_test]))
    print sigma
    for t in range(n_eval):
        ...
        kdes[t] = cal_nkde(emb_pred,emb_test,sigma)
